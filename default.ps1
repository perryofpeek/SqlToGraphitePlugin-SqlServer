properties {
  $Build_Solution = 'SqlToGraphitePlugin-SqlServer.sln'
  $Debug = 'Debug'
  $pwd = pwd
...
task Package {
   if ((Test-path -path $Build_Artifacts -pathtype container) -eq $false)
	...
	Copy-item .\src\SqlToGraphite.Plugin.SqlServer\output\SqlToGraphite.Plugin.SqlServer.dll $Build_Artifacts\
...
task Init {
	...
	$Product = "SqlToGraphite SqlServer Plugin $version";
	$Title = "SqlToGraphite SqlServer Plugin $version";
